...
AntCore.bolt_debug = CreateConVar("antcore_bolt_debug","0",FCVAR_ARCHIVE)
AntCore.bolt_grace = CreateConVar("antcore_bolt_grace","1.2",FCVAR_ARCHIVE)
...
hook.Add("ShouldCollide", "antcore_bolt_collision_filter", function(ent1, ent2)
	local bolt = nil
	local other = nil
	if IsValid(ent1) and ent1:GetClass() == "crossbow_bolt" and ent1.antcoreChip then
		bolt = ent1
		other = ent2
	elseif IsValid(ent2) and ent2:GetClass() == "crossbow_bolt" and ent2.antcoreChip then
		bolt = ent2
		other = ent1
	end
	if not IsValid(bolt) then return end
	if CurTime() < (bolt.antcoreSpawnTime or 0) + math.max(AntCore.bolt_grace:GetFloat(), 0) then return false end
	if bolt.antcoreNoCollide and bolt.antcoreNoCollide[other] then return false end
	if IsValid(other) and IsValid(bolt.antcoreChip) and isOwner(bolt.antcoreChip, other) then return false end
	if IsValid(other) and (other:IsPlayer() or other:IsNPC()) then return true end
	if IsValid(other) and other:IsWorld() then return true end
	if IsValid(other) and IsValid(other:GetPhysicsObject()) then return true end
	return false
end)
...
function AntCore.shootBolt(self, pos, vel, damage)
	...
	bolt.antcoreNoCollide = {}
	bolt.antcoreSpawnTime = CurTime()
	local BoltGrace = math.max(AntCore.bolt_grace:GetFloat(), 0)
	if BoltGrace > 0 then
		bolt:SetNotSolid(true)
	end
	bolt:Spawn()
	local Vel = Vector(vel[1],vel[2],vel[3])
	local RequestedPos = Vector(pos[1],pos[2],pos[3])
	-- crossbow_bolt can initialize its transform during Spawn; apply the
	-- requested muzzle transform afterward so E2 positions remain exact.
	bolt:SetCollisionGroup(COLLISION_GROUP_PROJECTILE)
	bolt:SetPos(RequestedPos)
	bolt:SetAngles(Vel:Angle())
	bolt:SetVelocity(vector_origin)
	bolt.antcoreLaunchTime = CurTime() + engine.TickInterval()
	...
	if BoltGrace > 0 then
		timer.Simple(BoltGrace, function()
			if IsValid(bolt) then
				bolt:SetNotSolid(false)
				bolt:SetCollisionGroup(COLLISION_GROUP_PROJECTILE)
			end
		end)
	end

	if AntCore.bolt_debug:GetBool() then
		local requestedPos = Vector(pos[1],pos[2],pos[3])
		print(string.format("[AntCore Bolt] requested=%s immediate=%s", tostring(requestedPos), tostring(bolt:GetPos())))
		timer.Simple(0, function()
			if IsValid(bolt) then
				print(string.format("[AntCore Bolt] next-tick=%s delta=%s", tostring(bolt:GetPos()), tostring(bolt:GetPos() - requestedPos)))
			end
		end)
	end
...
e2function void entity:noCollide(array entities)
	local ThisIsAntCoreBolt = IsValid(this) and this.antcoreChip == self
	if not IsValid(this) or (!ThisIsAntCoreBolt and not isOwner(self, this)) then return end
	
	self.prf = self.prf + #entities / 3
	for k,ent in pairs(entities) do
		if type(ent) == "Entity" and IsValid(ent) and (ThisIsAntCoreBolt or isOwner(self, ent)) then
			if ThisIsAntCoreBolt then
				this.antcoreNoCollide[ent] = true
			end
